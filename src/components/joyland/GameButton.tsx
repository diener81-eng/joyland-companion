...
interface GameButtonProps {
  ...
  // When true, make it more "available/important" visually (NO pulsing).
  highlight?: boolean;
}

const variantStyles: Record<GameButtonProps["variant"], string> = {
  ...
  return (
    <button
      ...
      className={cn(
        ...
        // ✅ Highlight = brighter + glow (NO animation)
        highlight && !disabled && [
          "brightness-125",
    ...
    >
      ...
      >
        <div className="flex items-center gap-1.5 shrink-0">
          <span className={cn("font-black", compact ? "text-sm" : "text-base")}>
            {letter}
          </span>
          <Icon
            className={cn("opacity-90", compact ? "h-3 w-3" : "h-4 w-4")}
            strokeWidth={2.5}
          />
        </div>
